...
def muveh_plot(adata, variants = [], plot_type = 'genotype', incl_oth = False, genomic_names = False, palette = None, **kwargs):
  ...
  if palette is None:
    palette = {'0' : '#80808030', '1' : 'yellow', '2' : 'red', '3' : 'purple'}
  ...
  if plot_type == 'genotype':
    sc.pl.umap(sc.AnnData(obs = pd.concat([adata.obs, pd.DataFrame.sparse.from_spmatrix(muveh_score_mut(adata)[:,variant_indices], \
               ...
               color = col_names, palette = palette, **kwargs)
